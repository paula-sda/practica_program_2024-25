...
def insertar_ubicacion(gestor):
    try:
        ...
        # Convertir a string antes de la inserción en la base de datos
        lat_str = str(lat)
        lng_str = str(lng)

        # Intentar insertar la ubicación en el gestor
        try:
            ubicacion_encontrada = gestor.insertar_nueva_ubicacion(lat_str, lng_str)
            if not ubicacion_encontrada:
                print(f"La ubicación '{lat_str}, {lng_str}' insertada correctamente.")  # Si la ubicación no existe se inserta correctamente
            else:
                print(f"La ubicación '{lat_str}, {lng_str}' ya existe.")  # Si la ubicación ya existe
